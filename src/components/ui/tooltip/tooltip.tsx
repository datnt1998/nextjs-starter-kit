"use client";

import * as React from "react";
import { Tooltip as BaseTooltip } from "@base-ui-components/react/tooltip";
import { cn } from "@/lib/utils";
import {
  tooltipPopupVariants,
  tooltipArrowVariants,
  type TooltipVariants,
} from "./tooltip.variants";

/**
 * Props for the Tooltip component.
 */
export interface TooltipProps
  extends Omit<
      React.ComponentPropsWithoutRef<typeof BaseTooltip.Root>,
      "children"
    >,
    TooltipVariants {
  /**
   * The element that triggers the tooltip
   */
  children: React.ReactElement;
  /**
   * The content to display in the tooltip
   */
  content: React.ReactNode;
  /**
   * The side of the trigger to place the tooltip
   */
  side?: "top" | "bottom" | "left" | "right";
  /**
   * Offset from the trigger element
   */
  sideOffset?: number;
  /**
   * Whether to show an arrow pointing to the trigger
   */
  showArrow?: boolean;
  /**
   * Additional className for the popup
   */
  className?: string;
  /**
   * Delay before showing the tooltip (in ms)
   */
  delay?: number;
  /**
   * Delay before hiding the tooltip (in ms)
   */
  closeDelay?: number;
}

const ArrowSvg = (props: React.ComponentProps<"svg">) => (
  <svg width="20" height="10" viewBox="0 0 20 10" fill="none" {...props}>
    <path
      d="M9.66437 2.60207L4.80758 6.97318C4.07308 7.63423 3.11989 8 2.13172 8H0V10H20V8H18.5349C17.5468 8 16.5936 7.63423 15.8591 6.97318L11.0023 2.60207C10.622 2.2598 10.0447 2.25979 9.66437 2.60207Z"
      className="fill-neutral-900 dark:fill-neutral-100"
    />
    <path
      d="M8.99542 1.85876C9.75604 1.17425 10.9106 1.17422 11.6713 1.85878L16.5281 6.22989C17.0789 6.72568 17.7938 7.00001 18.5349 7.00001L15.89 7L11.0023 2.60207C10.622 2.2598 10.0447 2.2598 9.66436 2.60207L4.77734 7L2.13171 7.00001C2.87284 7.00001 3.58774 6.72568 4.13861 6.22989L8.99542 1.85876Z"
      className="fill-neutral-700 dark:fill-neutral-300"
    />
    <path
      d="M10.3333 3.34539L5.47654 7.71648C4.55842 8.54279 3.36693 9 2.13172 9H0V8H2.13172C3.11989 8 4.07308 7.63423 4.80758 6.97318L9.66437 2.60207C10.0447 2.25979 10.622 2.2598 11.0023 2.60207L15.8591 6.97318C16.5936 7.63423 17.5468 8 18.5349 8H20V9H18.5349C17.2998 9 16.1083 8.54278 15.1901 7.71648L10.3333 3.34539Z"
      className="fill-neutral-700 dark:fill-neutral-300"
    />
  </svg>
);

/**
 * A flexible, accessible tooltip component.
 *
 * Built with Base-UI Tooltip primitives and styled with Tailwind CSS. Supports
 * multiple placement options, arrow indicator, and delay controls.
 * Fully accessible with proper ARIA attributes.
 *
 * @component
 *
 * @example
 * // Basic usage
 * <Tooltip content="This is a tooltip">
 *   <button>Hover me</button>
 * </Tooltip>
 *
 * @example
 * // With custom placement
 * <Tooltip content="Tooltip on the right" side="right">
 *   <button>Hover me</button>
 * </Tooltip>
 *
 * @example
 * // With arrow
 * <Tooltip content="Tooltip with arrow" showArrow>
 *   <button>Hover me</button>
 * </Tooltip>
 */
export const Tooltip = React.forwardRef<HTMLDivElement, TooltipProps>(
  (
    {
      children,
      content,
      side = "top",
      sideOffset = 10,
      showArrow = true,
      className,
      delay = 600,
      closeDelay = 0,
      ...props
    },
    ref
  ) => {
    return (
      <BaseTooltip.Root delay={delay} closeDelay={closeDelay} {...props}>
        <BaseTooltip.Trigger>{children}</BaseTooltip.Trigger>
        <BaseTooltip.Portal>
          <BaseTooltip.Positioner side={side} sideOffset={sideOffset}>
            <BaseTooltip.Popup
              ref={ref}
              className={cn(tooltipPopupVariants(), className)}
            >
              {showArrow && (
                <BaseTooltip.Arrow
                  className={cn(
                    tooltipArrowVariants(),
                    "data-[side=bottom]:top-[-8px] data-[side=left]:right-[-13px] data-[side=left]:rotate-90 data-[side=right]:left-[-13px] data-[side=right]:-rotate-90 data-[side=top]:bottom-[-8px] data-[side=top]:rotate-180"
                  )}
                >
                  <ArrowSvg />
                </BaseTooltip.Arrow>
              )}
              {content}
            </BaseTooltip.Popup>
          </BaseTooltip.Positioner>
        </BaseTooltip.Portal>
      </BaseTooltip.Root>
    );
  }
);

Tooltip.displayName = "Tooltip";

/**
 * TooltipProvider should be placed at the root of your app.
 * It provides shared state for all tooltips.
 */
export const TooltipProvider = BaseTooltip.Provider;
